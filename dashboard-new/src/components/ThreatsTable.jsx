...
import { Search, Filter, ChevronLeft, ChevronRight, AlertTriangle, Shield, FileWarning } from 'lucide-react';

function ThreatsTable({ threats, showAll = false }) {
  ...
  const filtered = threats.filter(t => {
    ...
    const matchesFilter = filterSeverity === 'all' || t.risk_level === filterSeverity;
    ...
    <div className="glass rounded-xl border border-white/5 overflow-hidden">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 border-b border-white/5">
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">{filtered.length} threats detected</p>
        </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5">
              ...
              <th className="text-left px-5 py-3 text-xs font-medium text-muted uppercase tracking-wider">Threat Type</th>
              ...
              paginated.map((threat, index) => (
                ...
                >
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      {getSeverityIcon(threat.risk_level)}
                      <div>
                        <p className="text-sm text-text-primary font-medium">{threat.file_name || 'Unknown'}</p>
                        <p className="text-xs text-muted truncate max-w-[200px]">{threat.file_path || ''}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-3">
                    <span className="text-sm text-text-primary">{threat.threat_type || 'Unknown'}</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${getSeverityStyle(threat.risk_level)}`}>
                      {threat.risk_level?.charAt(0).toUpperCase() + threat.risk_level?.slice(1)}
                    </span>
                  </td>
                  <td className="px-5 py-3">
                    <span className="text-sm text-muted">{formatBytes(threat.file_size)}</span>
                  </td>
                  <td className="px-5 py-3">
                    <span className="text-sm text-muted">{formatDate(threat.detected_at)}</span>
                  </td>
                  <td className="px-5 py-3">
